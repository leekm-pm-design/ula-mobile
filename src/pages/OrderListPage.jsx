...
const STATUS_FILTERS = ['전체', '등록', '접수', '배차', '운행', '완료', '취소'];
    ...
    <div className="min-h-dvh bg-gray-50 flex flex-col">
              ...
              {STATUS_FILTERS.map((s) => (
                ...
                >
                  <div className="flex flex-col items-center">
                    <span>{s}</span>
                    <span className={`text-[10px] ${statusFilter === s ? 'text-blue-100' : 'text-gray-400'}`}>
                      {statusCounts[s] || 0}
                    </span>
                  </div>
                </button>
      ...
      <div className="flex-1 px-4 pb-6 space-y-3">
        {loading ? (
          ...
          filtered.map((order) => (
            <CargoCard key={order.OrderNum} order={order} />
          ))
        )}
      </div>
